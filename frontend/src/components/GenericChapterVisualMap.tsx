...
export function GenericChapterVisualMap({
  ...
  const { pulse, pulseOuter, entranceAnims } = useVisualMapAnims(layout.length);
  ...
  return (
    <View onLayout={onLayout}>
      <View style={{ minHeight: CANVAS_H }}>
        ...
        <Image
          source={require("../../assets/map-bg/ch_generic.png")}
        ...
        {W > 0 && (
          <>
            ...
            {nodes.map((nd, idx) => {
              ...
              const bgColor =
                nd.status === "complete" ? chapterAccent + "32"
                : nd.status === "next"   ? tc + "28"
