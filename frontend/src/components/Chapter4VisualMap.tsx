  ...
  useEffect(() => {
    const anim = Animated.loop(
      Animated.sequence([
        ...
        Animated.timing(pulse, { toValue: 0.3, duration: 1000, useNativeDriver: false }),
      ]),
    );
    anim.start();
    return () => anim.stop();
              ...
              return (
                ...
                >
                  {nd.status === "complete" ? (
                    ...
                    isBossLocked ? (
                      // P10: locked WD final-stand shows shield silhouette (incoming surge feel)
                      <Ionicons name="shield-half-outline" size={Math.round(r * 0.7)} color={tc + "60"} />
                    ) : isWDLocked ? (
