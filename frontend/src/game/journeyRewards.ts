/**
 * journeyRewards.ts — Journey Map first-clear reward definitions (Chapters 1–10)
 *
 ...
 *   • University Credits: granted by University activities only; Journey story
 *     nodes (including converted challenge→story beats) give coins, not credits.
 *   • Sweep: handled separately by performSweep in store.tsx (unchanged).
 *
 * Reward values match the spec. The existing in-battle computePlayerXpReward
 * still fires as immediate feedback; journey node claims are ADDITIVE first-clear
 * bonuses tracked separately on the map.
...
// ── Node definitions (Chapters 1–10) ─────────────────────────────────────────
// "up to X player XP by stars" → 3★ = X, 2★ = 67%, 1★ = 33%.
// Story beats (isStory: true) give flat XP + coins; NO credits, NO shards.
// Credits come only from University activities — never from Journey story nodes.
...
/** Returns all node ids for a given chapter number (1–10). */
