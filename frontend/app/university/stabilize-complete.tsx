...
export default function StabilizeCompleteScreen() {
  ...
  // completeLotusLessonNode now writes obj_lotus_first_lesson to AsyncStorage
  // internally — it is the single source of truth for both player state and
  // the hub guide objective chain.  No separate completeObjective call needed.
  useEffect(() => {
    if (claimed) return;
    setClaimed(true);
    completeLotusLessonNode(LOTUS_NODE_ID).catch(() => {
      // Fail silently — node may already be complete; store is idempotent.
    });
