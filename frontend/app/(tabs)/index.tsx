...
// matching guide tutorial starts: it highlights the exact control that
// advances the chain and blocks every other navigation path until tapped.
...
const OBJECTIVE_GUIDES: Partial<Record<ObjectiveDef["id"], TutorialId>> = {
  ...
  obj_university_arrived:     "objGuideUniversity", // objective card CTA → /university
  obj_fading_apprentice_done: "objGuideUniversity",
  obj_recruit_preview:        "objGuideUniversity", // CTA routes to Recruitment Hall
  obj_lotus_visited:          "objGuideUniversity", // CTA routes to lessons
  obj_lotus_first_lesson:     "objGuideUniversity",
...
export default function RunHome() {
  ...
  // While an objective guide is active on this screen, all hub navigation
  // other than the guide's highlighted target is swallowed (forced path).
      ...
      <View style={styles.tutorialRow}>
        {realmUnlocked && (
          <Pressable
            ...
            onPress={() => { if (guideActive) return; router.push(ROUTES.KINGDOM); }}
            hitSlop={10}
        ...
        <View style={styles.guideOverlay} pointerEvents="box-none">
          <SystemObjectiveCard
            ...
            onPress={() => {
              ...
              // (objGuideUniversity); otherwise the tap is swallowed so the
              // forced path (e.g. Journey tab) stays the only way forward.
