...
- **Browser back on hubs (web):** hub screens that are replace-targets of gameplay flows (shift, mode/[id], university index, lotus-journal, world-event) use `useWebBackToHub(target)` — a web-only same-URL popstate sentinel that maps the browser back button to `router.replace(target)`, the same exit as the in-app arrow. Fresh back still works because each hub's arrow target IS the page it is normally pushed from. Skips pushing when `history.state.clinicaHubBack` is already set so refocus never stacks sentinels. Native back is untouched (no BackHandler/beforeRemove). Must run before early returns like all hooks.
